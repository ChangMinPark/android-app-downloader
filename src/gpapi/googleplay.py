...
class GooglePlayAPI(object):
    ...
    def uploadDeviceConfig(self):
        ...
        upload.deviceConfiguration.CopyFrom(
            self.deviceBuilder.getDeviceConfig())
    ...
    def login(self, email=None, password=None, gsfId=None, authSubToken=None):
        ...
        if email is not None and password is not None:
            ...
            encryptedPass = self.encryptPassword(email,
            ...
            raise LoginError(
                'Either (email,pass) or (gsfId, authSubToken) is needed')
    ...
    def bulkDetails(self, packageNames):
        ...
        return [
            None
            if not utils.hasDoc(entry) else utils.parseProtobufObj(entry.doc)
    ...
    def reviews(self,
        ...
        path = REVIEWS_URL + "?doc={}&sort={}".format(
